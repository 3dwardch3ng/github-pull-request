...
export async function prepareExistingDirectory(
  ...
  // Fetch URL does not match
    ...
    try {
      ...
      let branches: string[] = await git.branchList(false);
      ...
      if (ref) {
        ...
        if (ref.startsWith('refs/heads/')) {
          const upperName1: string = ref
            .toUpperCase()
            .substr('REFS/HEADS/'.length);
          const upperName1Slash: string = `${upperName1}/`;
          branches = await git.branchList(true);
          for (const branch of branches) {
            const upperName2: string = branch
              .substr('origin/'.length)
              .toUpperCase();
            const upperName2Slash: string = `${upperName2}/`;
            if (
